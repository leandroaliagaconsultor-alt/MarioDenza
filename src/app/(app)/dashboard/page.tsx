import {
  Building2, FileText, TrendingUp, AlertTriangle, Calendar, CreditCard, ExternalLink,
  Wallet, Clock, FileWarning, Plus, HandCoins,
} from "lucide-react";
...
function QuickAction({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white/80 p-4 shadow-sm transition hover:border-teal-300 hover:bg-teal-50/60">
...
export default async function DashboardPage() {
  ...
  const cobranzaPct = stats.cobranzaEsperado > 0
    ? Math.round((stats.cobranzaCobrado / stats.cobranzaEsperado) * 100)
    : 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <QuickAction href="/contratos/nuevo" icon={Plus} label="Nuevo contrato" />
        <QuickAction href="/pagos" icon={CreditCard} label="Pagos / Cobrar" />
        <QuickAction href="/liquidaciones" icon={HandCoins} label="Liquidaciones" />
        <QuickAction href="/aumentos" icon={TrendingUp} label="Aumentos" />
      </div>
      ...
      <section className="space-y-4">
        ...
        {/* Cobranza del mes + a liquidar a dueños */}
        <div className="rounded-xl border border-gray-200 bg-white/80 p-6 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm capitalize text-gray-500">{monthName}</span>
          </div>
          ...
          {stats.cobranzaEsperado === 0 ? (
            ...
            <>
              <div className="h-3 w-full overflow-hidden rounded-full bg-gray-100">
                <div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(100, cobranzaPct)}%` }} />
              </div>
              <p className="mt-2 text-xs text-gray-400">{cobranzaPct}% cobrado de lo esperado este mes</p>
              <div className="mt-4 grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
                ...
                <div className="rounded-lg bg-violet-50 px-3 py-2 ring-1 ring-violet-100">
                  <p className="text-xs font-medium uppercase text-violet-500">A liquidar a dueños</p>
                  <p className="mt-0.5 font-bold text-violet-700">{formatCurrency(stats.aLiquidar)}</p>
                </div>
              </div>
            </>
          )}
      ...
      <section className="space-y-4">
        ...
        {stats.expiredContracts.length > 0 && (
          <div className="rounded-xl border border-orange-200 bg-orange-50/40 p-6 shadow-sm">
            <h3 className="flex items-center gap-2 text-lg font-semibold text-gray-900">
        ...
        {/* Aumentos a ajustar */}
        <div className="rounded-xl border border-teal-200 bg-teal-50/30 p-6 shadow-sm">
          <h3 className="flex items-center gap-2 text-lg font-semibold text-gray-900">
              ...
              {adjustmentsByMonth.map((group) => (
                <div key={group.period}>
                  <div className="mb-2 flex items-center gap-2">
                    <h4 className="text-sm font-semibold capitalize text-gray-700">{group.label}</h4>
      ...
      {/* ===== VISIÓN GENERAL ===== */}
      <section className="space-y-4">
        <SectionTitle>Visión general</SectionTitle>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <OccupancyChart
